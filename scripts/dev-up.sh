#!/bin/sh
# Start the full local stack (API :8010, realtime :8001, frontend :3000/:3001).
# Logs land in /tmp/studysync-*.log. Stop with: scripts/dev-down.sh
set -e
cd "$(dirname "$0")/.."

(cd backend && nohup uv run uvicorn app.main:app --port 8010 > /tmp/studysync-api.log 2>&1 &)
...
for i in $(seq 1 30); do
  api=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8010/health || true)
  rt=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8001/health || true)
  [ "$api" = "200" ] && [ "$rt" = "200" ] && break
  sleep 1
done
echo "api=:8010($api) realtime=:8001($rt)"
